...
### Round 2 (September 14, 2026) — three additions

- [x] **Library — KPV and Ipamorelin as standalone entries.** Both previously existed only inside blends (`ghk-cu-kpv`, `cjc1295-ipamorelin`) so they couldn't be added to the stack alone. Now searchable on their own. All three citations verified against the NCBI E-utilities API (not just hand-written PMIDs): KPV = Dalmasso 2008 *Gastroenterology* + Kannengiesser 2008 *Inflamm Bowel Dis*; Ipamorelin = Raun 1998 *Eur J Endocrinol*. The KPV entry carries an explicit **route caveat** — KPV is commonly taken orally/topically, and the U-100 draw calculator assumes subcutaneous injection, so the dose range doesn't apply to those routes.
- [x] **Structured cycle data on all 16 library entries.** `cycleLength` was prose ("blocks of about 8–12 weeks") that no code could read. Added a parallel numeric `cycle: { onDays, offDays, continuous, note }` block to every entry, **derived from the already-accuracy-reviewed prose** — midpoints where the prose gives a range, with the reasoning recorded in `note`. Both `cycleLength` and `cycle` coexist. Shown in `PeptideDetail` as "Cycle used as the default" so it's visible where the form's pre-filled numbers come from.
  - Why per-peptide data mattered: Epithalon's real pattern is ~20 days on / ~150 off, so the flat 30-day default would have been off by 5×. Retatrutide and semaglutide are marked `continuous: true` because they aren't cycled at all. Tesamorelin/Ipamorelin's prose already stated 8 weeks on / 8 weeks off explicitly (56/56, no midpoint needed).
- [x] **BAC water tracker.** One punctured bottle at a time, stored in `peptide:bacwater` as `{ punctureDate, expiryDays }`, owned by `App.jsx`. New `src/components/BacWaterCard.jsx` pinned at the top of My Stack (visible whether or not the stack is empty — the bottle ages either way), plus a banner on Today that stays silent unless the bottle is close to or past its window. Default **28 days**, the commonly cited post-puncture beyond-use date for a preserved multi-dose vial — deliberately not 30. Editable, with a "New bottle (reset to today)" button.
  - **Reuses `daysSinceRecon` and `reconStatus` from `doseCalc.js` — no new date math was written.** Those two were already generic ("days since a date", "status against a window"); the 80%-of-window warn threshold means a 28-day bottle flips to "Expiring soon" on day 23 for free.
  - Decided *against* making BAC water a library entry (the original idea): the library is peptide reference data and `AddToStackForm` requires vial mg / dose range / schedule for the calculator, so an entry would pollute peptide search and carry meaningless dose and cycle fields.
- [x] **Off-cycle rest tracking.** `cycleStatusOf` in `scheduleCalc.js` gained a fourth state, **`"resting"`**, between `active` and `complete`, plus helpers `restDayOf` / `restDaysRemaining` / `nextCycleDate`. Stack items gained `restDays`, `continuous`, `cycleNumber`, and `cycleHistory`.
  - **Rest never auto-restarts.** When rest ends the item becomes `complete` — read as "ready to start" — with an explicit **Start next cycle** button (also available *during* rest as "Start next cycle early", because real life doesn't always match the plan). Tapping it rolls `cycleStart` to today, increments `cycleNumber`, and archives the finished cycle to `cycleHistory`. It **deliberately leaves `reconDate` alone** — you may be resuming on the same vial, and the recon card has its own date control.
  - Both forms gained a "Cycle & rest" section: a **continuous** checkbox (which hides the cycle-length and rest-days inputs entirely) and a rest-days input. `defaultsFor` now pre-fills cycle length from `cycle.onDays` instead of the old hardcoded 30.
  - Display: `StackItemCard` has a **two-phase progress bar** (the track spans the whole on+off block; on and rest segments fill independently) and Today gained a **Resting** section. Resting items are excluded from the due list so "Log dose" never appears for something you're off.
  - **Backward compatible:** an item with no `restDays` or `restDays: 0` skips `resting` entirely and behaves exactly as before.
...
- **If a deploy fails:** first check whether it's a real build error or a transient GitHub Pages error. Run `gh run view --log-failed`. If the build + artifact upload succeeded and it only failed at the `actions/deploy-pages` publish step with "Deployment failed, try again later," that's a transient Pages hiccup — just re-run: `gh workflow run deploy.yml` then `gh run watch`. (This happened on 2026-06-30 and a plain re-run fixed it.) Only debug the code if `npm ci` / `vite build` actually failed.
- **After a successful deploy, to refresh the installed iPhone app:** fully close it (swipe it away in the App Switcher) and reopen — twice. The service worker (`autoUpdate`) picks up the new version on the second relaunch. A private/incognito browser tab shows the true latest version if you want to confirm the deploy landed before wrestling with the phone cache. Saved data is preserved across updates.
